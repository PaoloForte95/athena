...
#include "plan2_planner/planners/tfd.hpp"
...
TFD::~TFD()
{
  RCLCPP_INFO(logger_, "Destroying plugin %s of type TFD", name_.c_str());
}
  ...
  std::string name)
{
  ...
  RCLCPP_INFO(logger_, "Configuring %s of type TFD", name.c_str());
...
void TFD::activate()
{
  RCLCPP_INFO(logger_, "Activating plugin %s of type TFD",name_.c_str());

  auto node = node_.lock();
  // Add callback for dynamic parameters
  _dyn_params_handler = node->add_on_set_parameters_callback(std::bind(&TFD::dynamicParametersCallback, this, _1));
}

void TFD::deactivate()
{
  RCLCPP_INFO(logger_, "Deactivating plugin %s of type TFD",
    name_.c_str());
...
{
  RCLCPP_INFO(
    logger_, "Cleaning up plugin %s of type TFD",name_.c_str());

}

plan2_msgs::msg::Plan TFD::computeExecutionPlan(const std::string & domain, const std::string & problem){
...
int status = system(("java -jar src/athena/plan2_planner/Planners/task_planner.jar tfd " + 
problem_type_ + " " +
domain + " " + 
problem + " " + 
plan_type_+ " " +
...
bool TFD::validateDomain(const std::string & domain){


}


bool TFD::validateProblem(const std::string & problem){

}


rcl_interfaces::msg::SetParametersResult TFD::dynamicParametersCallback(
...
PLUGINLIB_EXPORT_CLASS(plan2_planner::TFD, plan2_core::Planner)