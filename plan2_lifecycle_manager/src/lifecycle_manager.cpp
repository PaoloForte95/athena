...
#include "plan2_lifecycle_manager/lifecycle_manager.hpp"
...
using plan2_util::LifecycleServiceClient;
...
: Node("lifecycle_manager", options), diagnostics_updater_(this)
{
  ...
  init_timer_ = this->create_wall_timer(
    ...
    [this]() -> void {
      ...
      service_thread_ = std::make_unique<plan2_util::NodeThread>(executor);
    });
  diagnostics_updater_.setHardwareID("Plan2");
  diagnostics_updater_.add("Plan2 Health", this, &LifecycleManager::CreateActiveDiagnostic);
}
...
{
  if (system_active_) {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, "Plan2 is active");
  } else {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Plan2 is inactive");
  }
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(plan2_lifecycle_manager::LifecycleManager)
