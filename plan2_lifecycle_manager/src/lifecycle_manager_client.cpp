...
#include "plan2_lifecycle_manager/lifecycle_manager_client.hpp"
...
{
using plan2_util::geometry_utils::orientationAroundZAxis;
  ...
  std::shared_ptr<rclcpp::Node> parent_node)
{
  ...
  manager_client_ = std::make_shared<plan2_util::ServiceClient<ManageLifecycleNodes>>(
    manage_service_name_, node_);
  is_active_client_ = std::make_shared<plan2_util::ServiceClient<std_srvs::srv::Trigger>>(
