...
#ifndef PLAN2_LIFECYCLE_MANAGER__LIFECYCLE_MANAGER_HPP_
#define PLAN2_LIFECYCLE_MANAGER__LIFECYCLE_MANAGER_HPP_
...
#include "plan2_util/lifecycle_service_client.hpp"
#include "plan2_util/node_thread.hpp"
...
using plan2_msgs::srv::ManageLifecycleNodes;
/**
 * @class plan2_lifecycle_manager::LifecycleManager
 * @brief Implements service interface to transition the lifecycle nodes of
 * Plan2 stack. It receives transition request and then uses lifecycle
...
public:
  /**
   * @brief A constructor for plan2_lifecycle_manager::LifecycleManager
   * @param options Additional options to control creation of the node.
   */
  explicit LifecycleManager(const rclcpp::NodeOptions & options = rclcpp::NodeOptions());
  /**
   * @brief A destructor for plan2_lifecycle_manager::LifecycleManager
   */
  ~LifecycleManager();
...
protected:
  ...
  std::unique_ptr<plan2_util::NodeThread> service_thread_;
  ...
  /**
   * @brief function to check if the Plan2 system is active
   */
  ...
  std::map<std::string, std::shared_ptr<plan2_util::LifecycleServiceClient>> node_map_;
...
#endif  // PLAN2_LIFECYCLE_MANAGER__LIFECYCLE_MANAGER_HPP_
