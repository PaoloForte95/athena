...
#ifndef PLAN2_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
#define PLAN2_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
...
bool BtActionServer<ActionT>::on_configure()
{
  ...
  bt_ = std::make_unique<plan2_behavior_tree::BehaviorTreeEngine>(plugin_lib_names_);
...
void BtActionServer<ActionT>::executeCallback()
{
  ...
  plan2_behavior_tree::BtStatus rc = bt_->run(&tree_, on_loop, is_canceling, bt_loop_duration_);
  ...
  switch (rc) {
    case plan2_behavior_tree::BtStatus::SUCCEEDED:
      RCLCPP_INFO(logger_, "Goal succeeded");
      action_server_->succeeded_current(result);
      break;

    case plan2_behavior_tree::BtStatus::FAILED:
      RCLCPP_ERROR(logger_, "Goal failed");
      action_server_->terminate_current(result);
      break;

    case plan2_behavior_tree::BtStatus::CANCELED:
...
#endif  // PLAN2_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
