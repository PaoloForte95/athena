...
#include "athena_lifecycle_manager/lifecycle_manager_client.hpp"
...
{
using athena_util::geometry_utils::orientationAroundZAxis;
  ...
  std::shared_ptr<rclcpp::Node> parent_node)
{
  ...
  manager_client_ = std::make_shared<athena_util::ServiceClient<ManageLifecycleNodes>>(
    manage_service_name_, node_);
  is_active_client_ = std::make_shared<athena_util::ServiceClient<std_srvs::srv::Trigger>>(
