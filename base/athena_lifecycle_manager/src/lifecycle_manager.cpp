...
#include "athena_lifecycle_manager/lifecycle_manager.hpp"
...
using athena_util::LifecycleServiceClient;
...
: Node("lifecycle_manager", options), diagnostics_updater_(this)
{
  ...
  init_timer_ = this->create_wall_timer(
    ...
    [this]() -> void {
      ...
      service_thread_ = std::make_unique<athena_util::NodeThread>(executor);
    });
  diagnostics_updater_.setHardwareID("Athena");
  diagnostics_updater_.add("Athena Health", this, &LifecycleManager::CreateActiveDiagnostic);
}
...
{
  if (system_active_) {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, "Athena is active");
  } else {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Athena is inactive");
  }
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(athena_lifecycle_manager::LifecycleManager)
