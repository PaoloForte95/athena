...
#include <memory>
#include <limits>
#include "athena_bt_planner/planners/task_planner.hpp"
  ...
  std::shared_ptr<athena_util::OdomSmoother> odom_smoother)
{
  ...
  planning_problem_sub_ = node->create_subscription<athena_msgs::msg::PlanningProblem>(
    "planning_problem",
  ...
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  ...
  node->declare_parameter<std::string>("bt_package_dir", "athena_behavior_tree");
...
TaskPlanner::onLoop()
{
  ...
  try {
    ...
    athena_msgs::msg::Plan current_plan;
    blackboard->get<athena_msgs::msg::Plan>(plan_blackboard_id_, current_plan);
...
void
TaskPlanner::onPlanningProblemReceived(const athena_msgs::msg::PlanningProblem::SharedPtr problem)
{
  ActionT::Goal goal;
