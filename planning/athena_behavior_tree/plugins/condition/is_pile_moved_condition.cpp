#include "athena_behavior_tree/plugins/condition/is_pile_moved_condition.hpp"
...
: BT::ConditionNode(condition_name, conf), current_tick_(0)
{
  ...
  material_amount_client_ =  node_->create_client<material_handler_msgs::srv::GetMaterialAmount>(robot_id_ +"/get_material_amount");
}
...
BT::NodeStatus IsPileMovedCondition::tick()
{
  ...
  if(criteria_ == "Amount"){
    ...
    config().blackboard->get<std::string>(robot_id_+"_object_loaded", material);
    config().blackboard->get<std::string>(robot_id_+"_current_position", location);
    ...
    if(!material_amount_client_->wait_for_service(std::chrono::seconds(1))){
      RCLCPP_ERROR(node_->get_logger(), "Service 'get_material_amount' is not available.");
...
{
  factory.registerNodeType<athena_behavior_tree::IsPileMovedCondition>("IsPileMoved");
}
